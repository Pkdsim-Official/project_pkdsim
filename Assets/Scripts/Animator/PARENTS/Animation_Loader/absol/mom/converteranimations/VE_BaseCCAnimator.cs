using project_pkdsim.ANIMATOR.Parents.Vespiquen.Mom.converteranimations.IdleTo;
using project_pkdsim.ANIMATOR.Parents.Vespiquen.Mom.converteranimations.SittingTo;
using project_pkdsim.ANIMATOR.Parents.Vespiquen.Mom.converteranimations.WalkingTo;
using UnityEngine;

namespace project_pkdsim.ANIMATOR.Parents.Vespiquen.Mom.converteranimations
{
	public class AbsolParentMom_ConverterAnimatorController : MonoBehaviour 
	{
		private VEParentMom_SittingTo AbsolParentMomSittingto;
		
		private VEParentMom_WalkingTo AbsolParentMomWalkingto;
		
		private VEParentMom_IdleTo AbsolParentMomIdleto;

		public void VEParentMom_ConverterToAnimatorController()
		{
			AbsolParentMomWalkingto.VEParentMom_WalkingToController();

			AbsolParentMomIdleto.VEParentMom_IdleToController();
			
			AbsolParentMomSittingto.VEParentMom_SittingToController();	
		}
	}
}
...
namespace project_pkdsim.USys.HDSys.Modules.HOLIDAYSys.Modules.DataBase.ULoader.Assets.Modules.AssetsSorterI
{
	public class HDA_ULoader : MonoBehaviour
	{
		...
		public void InitializeHolidayAsset(string addressableKey = "")
		{
			if (string.IsNullOrEmpty(addressableKey))
			{
				InitializeAllHolidays(); // Load all holidays if no specific key is provided
			}
			else
			{
				InitializeCheckHolidayAsset(addressableKey); // Load a specific holiday asset
			}
		}
